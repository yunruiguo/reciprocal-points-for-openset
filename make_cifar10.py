...
def main():
    ...
    mkdir(PNG_PATH)
    mkdir(DATASET_PATH + '/png/train/')
    mkdir(DATASET_PATH + '/png/test/')

    print("Downloading {} dataset files to {}...".format(DATASET_NAME, DATASET_PATH))
    download('../data/' + 'cifar-10-python.tar.gz', IMAGES_LABELS_URL)

    train_labels = []
    train_filenames = []
    train_data = []
    for i in range(1, 6):
        data_file = '../data/cifar10' + '/cifar-10-batches-py/data_batch_{}'.format(i)
    ...
    test_filenames = []
    test_data = []
    with open('../data/cifar10' + '/cifar-10-batches-py/test_batch', 'rb') as fp:
        vals = pickle.load(fp, encoding='bytes')
    ...
    for idx, split in enumerate(splits):
        if not os.path.exists(DATA_DIR + '/split' + str(idx)):
            os.mkdir(DATA_DIR + '/split' + str(idx))
        ...
        for fake_index, _idx in enumerate(split):
            open_idx_to_label[fake_index+6] = CIFAR_CLASSES[int(_idx)]
            open_label_to_idx[CIFAR_CLASSES[int(_idx)]] = fake_index+6
        ...
        for _idx in range(10):
            if _idx not in split:
                idx_to_label[fake_index] = CIFAR_CLASSES[int(_idx)]
                label_to_idx[CIFAR_CLASSES[int(_idx)]] = fake_index
                fake_index += 1
        ...
        idx_to_label_ = '{}/split{}/idx_to_label.pkl'.format(DATA_DIR, idx)
        label_to_idx_ = '{}/split{}/label_to_idx.pkl'.format(DATA_DIR, idx)

        meta_ = '{}/split{}/meta.pkl'.format(DATA_DIR, idx)
        open_label_to_idx_ = '{}/split{}/open_label_to_idx.pkl'.format(DATA_DIR, idx)
        open_idx_to_label_ = '{}/split{}/open_idx_to_label.pkl'.format(DATA_DIR, idx)
        ...
        save_image_dataset(idx_to_label, idx_to_label_)
        save_image_dataset(label_to_idx, label_to_idx_)

        save_image_dataset(meta_dict, meta_)
        save_image_dataset(open_label_to_idx, open_label_to_idx_)
        save_image_dataset(open_idx_to_label, open_idx_to_label_)
...
def download(filename, url):
    if filename.endswith('.tgz') or filename.endswith('.tar.gz'):
        os.system('ls {} |  tar xzvf {} -C {}'.format(filename, filename, filename[:19]))
