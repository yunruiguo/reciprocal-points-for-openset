...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_folder_path", type=str,
                        help="path to dataset folder.", default="./")
    parser.add_argument("--dataset_folder", type=str,
                        help="name of folder where dataset details live.", default="cifar10_plus")
    ...
    parser.add_argument("--patience", type=int,
                        help="patience of lr scheduler", default=10)
    ...
    parser.add_argument("--checkpoint_folder_path", type=str,
                        help="folder where checkpoints will be saved", default="./")
    ...
    if args.msg == 'NONE':
        ...
        CKPT_BASE_NAME = args.msg + '_pat_' + str(
            args.patience) + '_div_' + args.divide + '_gap_' + args.gap + '_sched_' + args.lr_scheduler + '_latsize_' + str(
            args.latent_size) + '_numrp_' + str(args.num_rp_per_cls) + '_lambda_' + str(args.lamb) + '_gamma_' + str(
            args.gamma) + '_dataset_' + args.dataset_folder + '_' + str(args.lr).replace('0.', '') + '_' + str(
            args.batch_size) + '_' + str(args.img_size) + '_' + args.backbone_type
        LOGFILE_NAME = CKPT_BASE_NAME + '_logfile'
    ...
    logger = setup_logger('logger', formatter, "./" + "logging_cifar10+")
    ...
    if args.backbone_type == 'OSCRI_encoder':
        model = encoder32(latent_size=args.latent_size, num_classes=num_classes, num_rp_per_cls=args.num_rp_per_cls,
                          gap=args.gap == 'TRUE')
        ...
        model = encoder(backbone, VISUAL_FEATURES_DIM, latent_size=args.latent_size, num_classes=num_classes,
                        num_rp_per_cls=args.num_rp_per_cls, gap=args.gap == 'TRUE')
    else:
        raise ValueError(args.backbone_type + ' is not supported.')
    model_load = True
    if model_load:
        files = os.listdir(args.checkpoint_folder_path + CKPT_BASE_NAME)
        model_name = None
        for file in files:
            if file[-3:] == '.pt':
                ...
                model_name = file[:-3]
        model.load_state_dict(torch.load(args.checkpoint_folder_path + CKPT_BASE_NAME + '/' + model_name + '.pt'))
    model.cuda()

    num_params = count_parameters(model)
    print("Number of model parameters: " + str(num_params))
    ...
    for epoch in range(0, args.n_epochs):
        ...
        print("Learning rate: " + str(actual_lr))
        if actual_lr < 10 ** (-7):
            last_lr = True
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            # Compute RPL loss
            loss, open_loss, closed_loss, logits = compute_rpl_loss(model, outputs, labels, criterion, args.lamb,
                                                                    args.gamma, args.divide == 'TRUE')
        ...
        model.eval()
        used_running_loss, used_val_acc = evaluate_val(model, criterion, val_loader, args.gamma, args.lamb, args.divide,
                                                       logger)
        ...
        if used_val_acc > best_val_acc:
            ...
            for mover in models_to_move:
                os.replace(args.checkpoint_folder_path + CKPT_BASE_NAME + '/' + mover,
                           args.checkpoint_folder_path + CKPT_BASE_NAME + '/backups/' + mover)
